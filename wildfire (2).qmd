...
```{r}
wildfire_shp_data <- st_read(here("/Users/lehiwawong-frank/Desktop/DS422-Geocomputation/data/LANDSAT_USA_contiguous_and_Hawaii_7d/LANDSAT_USA_contiguous_and_Hawaii_7d.shp"))
```
...
```{r}
wildfire_shp_data <- wildfire_shp_data %>%
  mutate(ACQ_DATE = as.Date(ACQ_DATE))

fires_per_day <- wildfire_shp_data %>%
  group_by(ACQ_DATE) %>%
...
ggplot(fires_per_day, aes(x = ACQ_DATE, y = num_fires)) +
...
```{r}
fires_per_day_conf <- wildfire_shp_data %>%
  group_by(ACQ_DATE, CONFIDENCE) %>%
  summarise(num_fires = n(), .groups = "drop")

ggplot(fires_per_day_conf, aes(x = ACQ_DATE, y = num_fires, fill = CONFIDENCE)) +
  geom_col() +
  labs(
    title = "Number of Fires per Day by Confidence",
    x = "Date",
    y = "Number of Fires",
    fill = "Confidence"
  ) +
...
# Make sure your wildfire data is in sf with CRS 4326
...
# Optional: classify by confidence for coloring
...
# Define colors for confidence classes
conf_colors <- c("Low" = "#1f78b4",     # blue
                 "Medium" = "#ff7f00",  # orange
                 "High" = "#e31a1c")    # red

# Mapbox map with wildfire points
mapboxgl(style = mapbox_style("dark"),
  ...
  add_circle_layer(
    ...
    circle_color = match_expr(
      "conf_class",
      values = names(conf_colors),
      stops = unname(conf_colors)
    ),
    circle_radius = 5,
    circle_stroke_color = "#ffffff",
    circle_stroke_width = 1,
    circle_opacity = 0.7,
    tooltip = c("ACQ_DATE", "ACQ_TIME", "SATELLITE", "conf_class"),
    hover_options = list(circle_radius = 8,
                         circle_color = "#ffff99")
  ) %>%
  add_categorical_legend(
    legend_title = "Wildfire Confidence",
    values = names(conf_colors),
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1765321001510.kml",
...
```{r}
landfirehawaii<- read_csv(here("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv"))
```
...
landfire_sf <- landfirehawaii %>%
  st_as_sf(
    coords = c("longitude", "latitude"),  # coordinate columns
    crs = 4326,                           # WGS84
...
\`\`\`{r}library(ggplot2)} library(rnaturalearth) library(rnaturalearthdata)

# Get USA map

usa \<- ne_states(country = "United States of America", returnclass = "sf")

# Map fires with color by confidence and size by brightness

fires_sf \<- fires_sf %\>% mutate(confidence = factor(confidence, levels = c("low", "medium", "high")))

confidence_colors \<- c( "low" = "#1a9850", "medium" = "#fee08b", "high" = "#d73027" )

ggplot() + geom_sf(data = usa, fill = "gray90", color = "white") + geom_sf(data = fires_sf, aes(color = confidence, size = brightness), alpha = 0.7) + scale_color_manual(values = confidence_colors) + scale_size(range = c(0.5, 3)) + theme_minimal(base_size = 14) + labs( title = "LANDSAT Active Fires (USA + Hawaii, 7d)", color = "Confidence", size = "Brightness" )
